...
axis equal;
legend('true position', '$SO(2)$ UKF', '$SE_{1+L}(2)$ \textbf{left UKF}', ...
...
if cur_folder == "matlab"
    print(fig1, 'benchmarks/html/figures/benchmark_slam2d_01', ...
        '-dpng', '-r600')
    print(fig2, 'benchmarks/html/figures/benchmark_slam2d_02', ...
        '-dpng', '-r600')
    print(fig3, 'benchmarks/html/figures/benchmark_slam2d_03', ...
        '-dpng', '-r600')
end
disp(' ')
disp('Root Mean Square Error w.r.t. orientation (deg)');
disp("    -SO(2) x R^{2(1+L)} UKF: " + ukf_err_Rot);
disp("    -left SE_{1+L}(2) UKF  : " + ukf_left_err_Rot);
disp("    -right SE_{1+L}(2) UKF : " + ukf_right_err_Rot);
disp("    -EKF                   : " + ekf_err_Rot);
disp("    -IEKF                  : " + iekf_err_Rot);

disp(' ')
disp('Root Mean Square Error w.r.t. position (m)');
disp("    -SO(2) x R^{2(1+L)} UKF: " + ukf_err_p);
disp("    -left SE_{1+L}(2) UKF  : " + ukf_left_err_p);
disp("    -right SE_{1+L}(2) UKF : " + ukf_right_err_p);
disp("    -EKF                   : " + ekf_err_p);
